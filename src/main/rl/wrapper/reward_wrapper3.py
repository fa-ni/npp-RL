...
class RewardOption3Wrapper(gym.RewardWrapper):
    ...
    def reward(self, rew: float) -> float:
        ...
        if not done:
            result_reactor_wl = calculate_score_for_corridor_with_perfect_number(
                1500, 2500, 2100, self.state.full_reactor.reactor.water_level
            )
            result_reactor_pressure = calculate_score_for_corridor_with_perfect_number(
                0, 350, 175, self.state.full_reactor.reactor.pressure
            )
            ...
            result_condenser_pressure = calculate_score_for_corridor_with_perfect_number(
                0, 80, 40, self.state.full_reactor.condenser.pressure
            )
            result_corridor = (
                result_reactor_wl
