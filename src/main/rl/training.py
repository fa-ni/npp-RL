...
def train_agent(
    algorithm, environment, scenario_name: str, action_wrapper_name: str = None, obs_wrapper_name: str = None
):
    try:
        log_name_scenario = parse_scenario_name(scenario_name)
        log_name = f"{log_name_scenario}_{action_wrapper_name}_{obs_wrapper_name}_{algorithm.__name__}"
        environment.reset()
...
def train_all_scenarios(scenarios: list):
    # Algorithms
    ...
    for scenario in scenarios:
        ...
        for action_wrapper in ALL_ACTION_WRAPPERS:
            for observation_wrapper in ALL_OBSERVATION_WRAPPERS:
                ...
                for alg in algorithms:
                    train_agent(alg, vec_env_monitor, scenario, action_wrapper.__name__, observation_wrapper.__name__)
                    counter += 1
        # Single Wrapper
        for action_wrapper in ALL_ACTION_WRAPPERS:
            ...
            vec_env_monitor = VecMonitor(vec_env)
            for alg in algorithms:
                train_agent(alg, vec_env_monitor, scenario, action_wrapper.__name__, None)
                counter += 1
        for observation_wrapper in ALL_OBSERVATION_WRAPPERS:
            ...
            vec_env_monitor = VecMonitor(vec_env)
            for alg in algorithms:
                train_agent(alg, vec_env_monitor, scenario, None, observation_wrapper.__name__)
        ...
        for alg in algorithms:
            ...
            train_agent(alg, vec_env_monitor, scenario, None, None)
