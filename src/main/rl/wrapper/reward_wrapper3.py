...
class RewardOption3Wrapper(gym.RewardWrapper):
    ...
    def reward(self, rew: float) -> float:
        ...
        if not done:
            result_reactor_wl = calculate_reward_for_corridor(
                1500, 2500, 2100, self.state.full_reactor.reactor.water_level
            )
            result_reactor_pressure = calculate_reward_for_corridor(
            ...
            result_condenser_pressure = calculate_reward_for_corridor(
