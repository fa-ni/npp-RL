...
class RewardOption3Wrapper(gym.RewardWrapper):
    ...
    def reward(self, rew: float) -> float:
        # TODO Zahlen genauer definieren
        result_reactor_wl = calculate_reward_for_corridor(1500, 2500, 2100, self.state.full_reactor.reactor.water_level)
        result_reactor_pressure = calculate_reward_for_corridor(0, 350, 175, self.state.full_reactor.reactor.pressure)
        ...
        result_condenser_pressure = calculate_reward_for_corridor(0, 80, 40, self.state.full_reactor.condenser.pressure)
        result_corridor = (
            result_reactor_wl
        ...
        print(self.state.full_reactor.generator.power)
        result_power = calculate_roofed_reward(self.state.full_reactor.generator.power)
        print(f"power reward: {result_power}")
        return result_corridor / 5 + result_power * 0.8
