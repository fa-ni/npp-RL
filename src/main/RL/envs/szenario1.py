...
class Scenario1(Env):
    # TODO Check how to implement all scenarios more easy with wrappers
    # Szenario 1 with:
    # Observation Space: Option 1
    # Tpye of Model Option 2
    def __init__(self):
        # 1. moderator_percent 2. WPRPM
        self.action_space = MultiBinary(n=2)
        self.observation_space = Box(np.array([-1]).asytpe(np.float32), np.array([1]).asytpe(np.float32))
        self.length = 250
    ...
    def step(self, action):
        ...
        wp_rpm_setting = -1 if action[1] == 0 else 1
        ...
        calc_reward = self.state.full_reactor.generator.power / 750
