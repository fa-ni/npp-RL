...
from src.main.rl.utils.utils import WrapperMaker, delete_env_id, get_real_value
...
def plot_actions_taken(actions_taken: list, scenario_name: str) -> None:
    ...
    if parsed_scenario_name == "scenario1":
        for idx, item in enumerate(actions_positions):
            scaled_values.append([get_real_value(scaling_factors_scenario_1[idx], x) for x in item])
...
def plot_observations(observations_taken: list) -> None:
    ...
    for idx, item in enumerate(obs_positions):
        scaled_values_obs.append([get_real_value(current_obs_scaling_factors[idx], x) for x in item])
        # Plotting
...
def evaluate(scenario_name: str, path: str, alg: OnPolicyAlgorithm, wrapper: WrapperMaker) -> float:
    env_id = "TestEnv-v1"
...
path = "models/scenario2/full_test_26_02_wo_automation/scenario2_ActionSpaceOption2Wrapper_ObservationOption2Wrapper_RewardOption2Wrapper_A2C_full_test_26_02_wo_automation_1/best_model.zip"
scenario, alg, wrapper_maker = parse_information_from_path(path)
evaluate(scenario, path, alg, wrapper_maker)
