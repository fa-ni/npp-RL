...
def test_step(set_up):
    ...
    assert len(actual_step_one[0]) == 11
    assert len(actual_step_two[0]) == 11
    np.testing.assert_almost_equal(
        actual_step_one[0], np.array([-1, 0.0005, -1, -0.00025, -1, -0.987, 1, 0.6, 1, 1, 1])
    )
    np.testing.assert_almost_equal(
        actual_step_two[0], np.array([-1, 0.0, -0.99868, -0.00036, -1, -0.968, 0.98, 0.6, 1, 1, 1]), 5
    )
    np.testing.assert_almost_equal(
        actual_step_three[0], np.array([-0.995, -0.00089, -0.99611, -0.00064, -0.99980, -0.946, 0.96, 0.6, 1, 1, 1]), 5
    )
...
def test_reset(set_up):
    ...
    np.testing.assert_almost_equal(actual, np.array([-1, 0, -1, 0, -1, -1, 1, 0.6, 1, 1, 1]))
